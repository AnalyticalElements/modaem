...
private
  ...
  real (4) :: rgrx1,rgry1                ! world coordinates of lower left of graphics window
  real (4) :: rgrx2,rgry2                ! world coordinates of upper right of graphics window
  real (4) :: rgrmax,rgrmin              ! max and min of any scalar field plotted on the grid
  integer (4) :: ngrx,ngry               ! number of grid points in x and y directions
  real (4),dimension(MGSZ) :: rgrxg      ! grid x-coordinates
  real (4),dimension(MGSZ) :: rgryg      ! grid y-coordinates
  real (4),dimension(MGSZ,MGSZ) :: rgrid ! grid z-values
  real (4) :: rgrdx,rgrdy                ! grid point spacing in x and y directions
...
subroutine CTR_ReadGRD(iInLU)
  ...
  integer (kind=ModAEM_Integer),intent(in) :: iInLU
...
subroutine CTR_DrawContours(iOutLU,rZMin,rZIncr,rZMax)
  integer (kind=ModAEM_Integer),intent(in) :: iOutLU
  real (kind=ModAEM_Real),intent(in) :: rZMin,rZIncr,rZMax
