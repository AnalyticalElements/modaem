...
public :: cILS_InfluenceP, &
  ...
  real (kind=ModAEM_Real) :: END_TOLERANCE = 1.0e-4_ModAEM_Real

contains

function cILS_InfluenceP(cMapZ,cZL) result(cP)
...
function cILS_InfluenceW(cMapZ,cZL) result(cW)
  ...
  cW(1,1) = (rHALF / rPI) * cZL * &
          abs(rTWO * cZL) * log((cMapZ-rONE)/(cMapZ+rONE))
...
function cILS_InfluenceF(cMapZ1,cMapZ2,cZL) result(cF)
  ...
  !!
  ! [ ARGUMENTS ]
  complex (kind=ModAEM_Real),intent(in) :: cMapZ1,cMapZ2,cZL
  ! [ RETURN VALUE ]
  complex (kind=ModAEM_Real),dimension(1,1) :: cF
  ! [ LOCALS ]
  complex (kind=ModAEM_Real),dimension(1,1) :: cP1,cP2
  real (kind=ModAEM_Real) :: rXInt
  complex (kind=ModAEM_Real),dimension(1,1) :: cBC
  ...
  if ( (aimag(cMapZ1) >= rZERO .and. aimag(cMapZ2) < rZERO ) .or. &
    ...
    if ( rXInt < -rOne ) then
      if ( aimag(cMapZ1) < rZERO ) then
        cBC = -cmplx(rZERO,rTWO*abs(cZL),ModAEM_Real)
      else
        cBC = cmplx(rZERO,rTWO*abs(cZL),ModAEM_Real)
      endif
    else if ( rXInt < rONE ) then
      if ( aimag(cMapZ1) < rZERO ) then
        cBC = cmplx(rZERO,(rXInt-rONE)*abs(cZL),ModAEM_Real)
      else
        cBC = -cmplx(rZERO,(rXInt-rONE)*abs(cZL),ModAEM_Real)
      end if
    else 
...
function cILS_InfluenceG() result(cR)
  ...
  ! [ ARGUMENTS ]
  ! [ RETURN VALUE ]
  complex (kind=ModAEM_Real),dimension(1,1) :: cR
  ! [ LOCALS ]
...
function cILS_InfluenceQ(cZL) result(cQ)
  ...
  !!
  ! [ ARGUMENTS ]
  complex (kind=ModAEM_Real),intent(in) :: cZL
  ! [ RETURN VALUE ]
