...
function cIDP_InfluenceP(cMapZ) result(cP)
  ...
  !!
  ! [ ARGUMENTS ]
  complex (kind=ModAEM_Real),intent(in) :: cMapZ
  ! [ RETURN VALUE ]
  complex (kind=ModAEM_Real),dimension(3,1) :: cP
  ! [ LOCALS ]
  complex (kind=ModAEM_Real) :: cF,cFStar,cG,cCauchy
...
function cIDP_InfluenceW(cMapZ) result(cW)
  ...
  complex (kind=ModAEM_Real),intent(in) :: cMapZ
  ! [ RETURN VALUE ]
  complex (kind=ModAEM_Real),dimension(3,1) :: cW
  ! [ LOCALS ]
  complex (kind=ModAEM_Real) :: cDF,cDFStar,cDG,cCauchy
...
function cIDP_InfluenceF(cMapZ1,cMapZ2) result(cF)
  ...
  !!
  ! [ ARGUMENTS ]
  complex (kind=ModAEM_Real),intent(in) :: cMapZ1,cMapZ2
  ! [ RETURN VALUE ]
  complex (kind=ModAEM_Real),dimension(3,1) :: cF
  ! [ LOCALS ]
  complex (kind=ModAEM_Real),dimension(3,1) :: cP1,cP2
  real (kind=ModAEM_Real) :: rXInt
  complex (kind=ModAEM_Real),dimension(3,1) :: cBC
  ...
  if ( (aimag(cMapZ1) >= rZERO .and. aimag(cMapZ2) < rZERO ) .or. &
    ...
    if ( rXInt > -rONE .and. rXInt < rONE ) then
      if ( aimag(cMapZ1) < rZERO ) then
        cBC(1,1) = cmplx(rZERO, -rHALF * (rXInt-rONE), ModAEM_Real )
        cBC(2,1) = cmplx(rZERO,  rXInt*rXInt - rONE,   ModAEM_Real )
        cBC(3,1) = cmplx(rZERO,  rHALF * (rXInt+rONE), ModAEM_Real )
      else
        cBC(1,1) = -cmplx(rZERO, -rHALF * (rXInt-rONE), ModAEM_Real )
        cBC(2,1) = -cmplx(rZERO,  rXInt*rXInt - rONE,   ModAEM_Real )
        cBC(3,1) = -cmplx(rZERO,  rHALF * (rXInt+rONE), ModAEM_Real )
  ...
  cF = cmplx(aimag(cF),-real(cF),ModAEM_Real)

  return
end function cIDP_InfluenceF

function cIDP_InfluenceG(cMapZ) result(cR)
  ...
  complex (kind=ModAEM_Real),intent(in) :: cMapZ
  ! [ RETURN VALUE ]
  complex (kind=ModAEM_Real),dimension(3,1) :: cR
  ! [ LOCALS ]
...
function cIDP_InfluenceQ() result(cQ)
  ...
  ! [ ARGUMENTS ]
  ! [ RETURN VALUE ]
  complex (kind=ModAEM_Real) :: cQ(3,1)
  ! [ LOCALS ]
...
function cIDP_InfluenceJ(rX) result(cJ)
  ...
  !!
  ! [ ARGUMENTS ]
  real (kind=ModAEM_Real),intent(in) :: rX
  ! [ RETURN VALUE ]
  complex (kind=ModAEM_Real) :: cJ(3,1)
  ! [ LOCALS ]
  real (kind=ModAEM_Real) :: rTemp
  
  rTemp = -rHALF * (rX - rONE)
  cJ(1,1) = cmplx(rTemp,rTemp,ModAEM_Real)
  rTemp =  rX*rX - rONE
  cJ(2,1) = cmplx(rTemp,rTemp,ModAEM_Real)
  rTemp =  rHALF * (rX + rONE)
  cJ(3,1) = cmplx(rTemp,rTemp,ModAEM_Real)
