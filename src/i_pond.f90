...
function cIPD_InfluenceP(cZ,cZC,rRad) result(cP)
  ...
  !!
  ! [ ARGUMENTS ]
  complex (kind=ModAEM_Real),intent(in) :: cZ,cZC
  real (kind=ModAEM_Real),intent(in) :: rRad
  ! [ RETURN VALUE ]
  complex (kind=ModAEM_Real),dimension(1,1) :: cP
  ! [ LOCALS ]
  real (kind=ModAEM_Real) :: rR

  rR = abs(cZ-cZC)
  if ( rR <= rRad ) then          
    ! Inside function
    cP(1,1) = cmplx(rONE_FOURTH * ( rR*rR - rRad*rRad ),rZERO,ModAEM_Real) 
  else                            
...
function cIPD_InfluenceW(cZ,cZC,rRad) result(cW)
...
function cIPD_InfluenceF(cZ1,cZ2,cZC,rRad) result(cF)
  ...
  !!
  ! [ ARGUMENTS ]
  complex (kind=ModAEM_Real),intent(in) :: cZ1,cZ2,cZC
  real (kind=ModAEM_Real),intent(in) :: rRad
  ! [ RETURN VALUE ]
  complex (kind=ModAEM_Real),dimension(1,1) :: cF
  ! [ LOCALS ]
  complex (kind=ModAEM_Real) :: cZ,cZ0,cZi,cR1,cR2,cV12,cV21,cZInt1, &
                                   cZInt2,cZCZ0,cZChord
  real (kind=ModAEM_Real) :: rR1,rR2,rA,rB,rC,rD,rZ0ZChord,rDZ
  ...
  elseif ( rR1<rRad .and. rR2>rRad ) then
  ...
  elseif ( rR2<rRad .and. rR1>rRad ) then
    ...
    if ( abs(aimag(cZ)) >= rRad .or. abs(real(cZ))>1.0 ) then
...
function rIPD_InfFlowInside(cZ1,cZ2,cZC) result(rF)
  ...
  ! [ LOCALS ]
  complex (kind=ModAEM_Real) :: cR,cS
  real (kind=ModAEM_Real) :: rDR1,rDR2,rDS1,rDS2,rDW3
...
function rIPD_InfFlowOutside(cZ1,cZ2,cZC,rRad) result(rF)
  !! real function cIPD_InfFlowInside
  !!
  !! Computes the integrated flow influence function for a single
  !! pond with center cZC and radius rRad between the complex coordinates
  ...
  real (kind=ModAEM_Real),dimension(1,1) :: rF
  ! [ LOCALS ]
  complex (kind=ModAEM_Real) :: cP1,cP2
  real (kind=ModAEM_Real) :: rX1,rX2,rY1,rY2,rXi,rBC
...
function cIPD_InfluenceG(cZ,cZC,rRad) result(cG)
  !! real function cIPD_InfFlowInside
  !!
  ...
  rR = abs(cZ-cZC)
  if ( rR <= rRad ) then          ! Inside function
    cG = rONE
...
function cIPD_InfluenceQ(rRad) result(cQ)
